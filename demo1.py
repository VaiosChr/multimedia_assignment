...
for i in range(0, y1.shape[0], 8):
    for j in range(0, y1.shape[1], 8):
        ...
        y1_new[i:i+8, j:j+8] = quantizeJPEG(y1_new[i:i+8, j:j+8], qTableL, 0.6)
        y2_new[i:i+8, j:j+8] = quantizeJPEG(y2_new[i:i+8, j:j+8], qTableL, 5)

        # Dequantize the coefficients
        y1_new[i:i+8, j:j+8] = dequantizeJPEG(y1_new[i:i+8, j:j+8], qTableL, 0.6)
        y2_new[i:i+8, j:j+8] = dequantizeJPEG(y2_new[i:i+8, j:j+8], qTableL, 5)
...
for i in range(0, cr1.shape[0], 8):
    for j in range(0, cr1.shape[1], 8):
        ...
        # Quantize the coefficients
        cr1_new[i:i+8, j:j+8] = quantizeJPEG(cr1_new[i:i+8, j:j+8], qTableC, 0.6)
        cb1_new[i:i+8, j:j+8] = quantizeJPEG(cb1_new[i:i+8, j:j+8], qTableC, 0.6)

        # Dequantize the coefficients
        cr1_new[i:i+8, j:j+8] = dequantizeJPEG(cr1_new[i:i+8, j:j+8], qTableC, 0.6)
        cb1_new[i:i+8, j:j+8] = dequantizeJPEG(cb1_new[i:i+8, j:j+8], qTableC, 0.6)
...
for i in range(0, cr2.shape[0], 8):
    for j in range(0, cr2.shape[1], 8):
        ...
        # Quantize the coefficients
        cr2_new[i:i+8, j:j+8] = quantizeJPEG(cr2_new[i:i+8, j:j+8], qTableC, 5)
        cb2_new[i:i+8, j:j+8] = quantizeJPEG(cb2_new[i:i+8, j:j+8], qTableC, 5)

        # Dequantize the coefficients
        cr2_new[i:i+8, j:j+8] = dequantizeJPEG(cr2_new[i:i+8, j:j+8], qTableC, 5)
        cb2_new[i:i+8, j:j+8] = dequantizeJPEG(cb2_new[i:i+8, j:j+8], qTableC, 5)
